...
НАСТРОЙКА:
  ...
  3. Заполните SOURCE_CHANNEL_ID и DEST_CHANNEL_ID ниже.
  4. python bot.py
"""
...
# ─── НАСТРОЙКИ ────────────────────────────────────────────────────────────────
TOKEN             = "8696143142:AAGmtXacwCLm9UM7msOhcdb8yFKk0Wn-qkA"
SOURCE_CHANNEL_ID = -1003895519984  # ← ID канала-источника, напр. -1001234567890
DEST_CHANNEL_ID   = -1003658657304  # ← ID канала-назначения, напр. -1009876543210
...
async def close_message(context: ContextTypes.DEFAULT_TYPE):
    """Редактирует сообщение в канале-назначении через 2 часа."""
...
async def handle_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # Копируем как обычный текст/медиа (не пересылка)
        if msg.text:
            ...
            # Остальные типы (стикеры, голосовые и т.д.) — копируем через copy_message
